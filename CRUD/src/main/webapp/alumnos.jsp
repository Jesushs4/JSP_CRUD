...
<%      String url = "jdbc:mysql://localhost:3306/alumnos";
        ...
        boolean añadirAlumno = Boolean.parseBoolean(request.getParameter("añadirAlumno")); 
        boolean modificarAlumno = Boolean.parseBoolean(request.getParameter("modificarAlumno")); 
        boolean borrarAlumno = Boolean.parseBoolean(request.getParameter("borrarAlumno")); 
        ...
        if (modificarAlumno) {
            try {
        aservice.update(id,nombre,apellidos);
            } catch (Exception e) {
                out.println("Error: no se pudo modificar el alumno, asegurese de introducir bien los datos");
            }
        }
        if (borrarAlumno) {
            try {
            aservice.delete(id);
                ...
                out.println("Error: no se pudo borrar el alumno, asegurese de introducir bien los datos");
            }
        }
%>
...
<div class="container text-center">
    <div class="row pt-5">
        <div class="col">
            <form method="POST" action="menuAlumno.jsp">
                <input type="hidden" name="añadir" value="true">
                <button type="submit" class="btn btn-success btn-lg btn-block">Añadir alumno</button>
            </form>
        </div>
        <div class="col">
            <form method="POST" action="menuAlumno.jsp">
                <input type="hidden" name="modificar" value="true">
                <button type="submit" class="btn btn-warning btn-lg btn-block">Modificar alumno</button>
            </form>
        </div>
        <div class="col">
            <form method="POST" action="menuAlumno.jsp">
                <input type="hidden" name="borrar" value="true">
                <button type="submit" class="btn btn-danger btn-lg btn-block">Eliminar alumno</button>
            </form>    
        </div>
    </div>
    <div class="pt-5 pb-5">
